...
outputs it as a bare `version` (e.g. `1.2.3`). Pair it with the release actions
...
`v` prefix), so this resolver only reads the version.

## Requirements

- Python 3.11+ on the runner (uses the standard-library `tomllib` to parse the
  file). uv projects run modern Python, so this is normally already present.
